...
#  2023-03-29 Update to Python 3 (by @fakerybakery on GitHub)
...
def add_tarfile(tarinfo, file, tar, mode=0o100644):
    tarinfo.mode = mode
...
def make_bin(basename, filelist, type, kver, sign=0, jailbreak=0):
    ...
    if sign:
        ...
        fs = os.fdopen(fd,"wb")
        fs.write(SIGN_KEY.encode('utf-8'))
        fs.close()
    ...
    if jailbreak:
        ...
        fs = os.fdopen(fd,"wb")
        fs.write(INSTALL_SCRIPT.encode('utf-8'))
        fs.close()
    ...
    fs = os.fdopen(fd,"wb")
    fs.write(dat_list.encode('utf-8'))
    fs.close()
...
def convert_bin(basename, tgz_fname, type, kver):
    ...
    header = struct.pack("<4sIIHBB", sig.encode('utf-8'), 0x0, 0x7fffffff, kver, 0, 0x13) #signature, fromVer, toVer, devCode, optional
    of.write(header)
    of.write(md(s_md5(f)))
    of.write(b"\0" * (BLOCK_SIZE - of.tell()))
    of.write(md(f))
    of.close()
